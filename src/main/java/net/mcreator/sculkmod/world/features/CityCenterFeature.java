...
public class CityCenterFeature extends Feature<NoneFeatureConfiguration> {
	public static final CityCenterFeature FEATURE = (CityCenterFeature) new CityCenterFeature().setRegistryName("sculk_mod:city_center");
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("sculk_mod", "city_center_1"));
		if (template == null)
			return false;
